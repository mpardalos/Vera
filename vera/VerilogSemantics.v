...
Ltac unpack_defined_value_for :=
  ...
  Global Instance match_on_proper :
    Proper (pointwise_relation Verilog.variable iff ==> eq ==> eq ==> iff) match_on.
  Proof. repeat intro. subst. crush. Qed.

  (* Add Parametric Morphism : defined_value_for
   *   with signature (pointwise_relation Verilog.variable iff) ==> eq ==> iff
   *   as defined_value_for_match_mor.
   * Proof. Admitted.
   * 
   * Add Parametric Morphism C : (@RegisterState.defined_value_for C)
   *   with signature (match_on C) ==> iff
   *   as defined_value_for_iff_mor.
   * Proof. Admitted. *)
